...
class Models(object):
    ...
    def inception_resnet2(self):
        ...
        design_model = InceptionResNetV2(include_top=False, weights='imagenet', input_tensor=input_tensor)
        # FC層を構築
    ...
    def xception(self):
        input_tensor = Input(shape = (self.h,self.w,self.ch))
        design_model = Xception(include_top=False, weights='imagenet', input_tensor=input_tensor)
        # FC層を構築
        top_model = Sequential()
        top_model.add(Flatten(input_shape=design_model.output_shape[1:]))
        top_model.add(Dense(256, activation='relu'))
    ...
    def inception3(self):
        input_tensor = Input(shape = (self.h,self.w,self.ch))
        design_model = InceptionV3(include_top=False, weights='imagenet', input_tensor=input_tensor)
        # FC層を構築
        top_model = Sequential()
        top_model.add(Flatten(input_shape=design_model.output_shape[1:]))
        top_model.add(Dense(256, activation='relu'))
    ...
    def dense121(self):
        input_tensor = Input(shape = (self.h,self.w,self.ch))
        design_model = DenseNet121(include_top=False, weights='imagenet', input_tensor=input_tensor)
        # FC層を構築
        top_model = Sequential()
        top_model.add(Flatten(input_shape=design_model.output_shape[1:]))
        top_model.add(Dense(256, activation='relu'))
    ...
    def dense169(self):
        input_tensor = Input(shape = (self.h,self.w,self.ch))
        design_model = DenseNet169(include_top=False, weights='imagenet', input_tensor=input_tensor)
        # FC層を構築
        top_model = Sequential()
        top_model.add(Flatten(input_shape=design_model.output_shape[1:]))
        top_model.add(Dense(256, activation='relu'))
    ...
    def dense201(self):
        input_tensor = Input(shape = (self.h,self.w,self.ch))
        design_model = DenseNet201(include_top=False, weights='imagenet', input_tensor=input_tensor)
        # FC層を構築
        top_model = Sequential()
        top_model.add(Flatten(input_shape=design_model.output_shape[1:]))
        top_model.add(Dense(256, activation='relu'))
    ...
    def resnet50(self):
        input_tensor = Input(shape = (self.h,self.w,self.ch))
        design_model = ResNet50(include_top=False, weights='imagenet', input_tensor=input_tensor)
        # FC層を構築
        top_model = Sequential()
        top_model.add(Flatten(input_shape=design_model.output_shape[1:]))
        top_model.add(Dense(256, activation='relu'))
    ...
    def vgg19(self):
        input_tensor = Input(shape = (self.h,self.w,self.ch))
        design_model = VGG19(include_top=False, weights='imagenet', input_tensor=input_tensor)
        # FC層を構築
        top_model = Sequential()
        top_model.add(Flatten(input_shape=design_model.output_shape[1:]))
        top_model.add(Dense(256, activation='relu'))
    ...
    def vgg_model(self):
        model = Sequential()
        model.add(Conv2D(64, (3, 3), padding='valid', input_shape=(self.h, self.w, self.ch)))
    ...
    def test_model(self):
        model = Sequential()
        model.add(Conv2D(32, (3, 3), padding='valid', input_shape=(self.h, self.w, self.ch)))
        model.add(Activation('relu'))
...
class Learning(Training):
    ...
    def learning_model(self):
        ...
        es_cb = EarlyStopping(monitor='loss', min_delta=0, patience=20, verbose=1, mode='auto')
